...
from naive_autosegmentation import compute_vertex_assignments
...
for feature in facial_landmark_groups_keys:
    ...
    non_local_features = np.delete(np.arange(lm_sample_i_original.shape[1]), local_features)

    
    # feature_related_indices, feature_unrelated_vertices = compute_vertex_assignments(flame.V, flame.F, local_features, non_local_features, K=1)
    ...
    # plt.scatter(flame.V[feature_related_indices, 0], flame.V[feature_related_indices, 1], s=1, c='red', alpha=0.5)
    ...
    for frame_i in range(weight.shape[0]):
        ...
        for i in range(100):
            ...
            loss_local = torch.mean((landmarks3d[0, local_features] - torch.from_numpy(lm_sample_i_original[frame_i, local_features, :]))**2)
            loss_non_local = torch.mean((landmarks3d[0, non_local_features] - torch.from_numpy(lm_neutral[non_local_features, :]))**2 * 5)
