...
for feature in facial_landmark_groups_keys:
    ...
    for frame_i in range(weight.shape[0]):
        ...
        for i in range(100):
            ...
            loss_local = torch.mean((vertices[0, feature_related_indices] - torch.from_numpy(V_sample_i_original[frame_i, feature_related_indices, :]))**2)
            loss_non_local = torch.mean((vertices[0, feature_unrelated_vertices] - torch.from_numpy(V_neutral[feature_unrelated_vertices, :]))**2) * 10
    ...
    save_dir = f"/Users/evanpan/Documents/GitHub/ManifoldExploration/experiments/full_face_bs_test_freeze_landmarks_and_{K}_ajacent_200_videos/"
