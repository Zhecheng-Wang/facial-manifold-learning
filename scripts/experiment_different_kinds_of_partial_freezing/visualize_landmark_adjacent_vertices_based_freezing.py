...
directory_list = [
    ...
    "/Users/evanpan/Documents/GitHub/ManifoldExploration/experiments/full_face_bs_test_freeze_landmarks_and_5_ajacent_test_batch",
...
for directory_i in range(len(directory_list)):
    for feature_i in range(len(masks_we_care_about)):
        ...
        feature = masks_we_care_about[feature_i]
        directory = directory_list[directory_i]

        V_for_feature_i = []
        partially_frozened_model_weights = os.path.join(directory, "bs_for_" + feature + ".npy")
        try:
        ...
        # note that the polyscope translation is y-down, so the top animation will be "full_face_bs_test_freeze_landmarks"
        flame.translation = np.array([float(feature_i) * 0.2, float(directory_i) * 0.2, 0.0], dtype=np.float32)
...
animator = MultiMeshAnimator(input_list, offset_distance=0.0)
animator.run()
