...
weight_optimizer = BatchedManualAdam(current_batch_size, [(51, )], lr=0.05, device=device, dtype=torch.double)
direction_optimizer =  BatchedManualAdam(51, [(103,) ,], lr=EM_LEARNING_RATE, device=device, dtype=torch.double)

shape_params = torch.zeros([51, 100]).to(device).double()
tex_params = torch.zeros([51, 50]).to(device).double()
pose_params = torch.zeros([51, 3]).to(device).double()
eye_pose_params = torch.zeros([51, 6]).to(device).double()
...
eye_pose_params_frames = torch.zeros((weight.shape[0], 6), device=device, dtype=torch.double)  # (Frames, 6)
...
for i in range(0, EM_ITERATIONS):
    ...
    for fitting_iter in range(0, 10000):
        ...
        loss = recon_loss_latent + recon_loss_geometry 
        loss.backward()
        weight_optimizer.step([FACS_weights], [FACS_weights.grad])
        FACS_weights.grad.zero_() # reset grad
        if fitting_iter % 10 == 0:
            print(f"EM iteration {i}, fitting iteration {fitting_iter}: recon loss latent: {recon_loss_latent.item()}, recon loss geometry: {recon_loss_geometry.item()}, l1 loss: {l1_loss.item()}")
    losses_weights_recon.append(recon_loss.item())
        
    print(f"weights -> EM iter: {i}, recon loss: {recon_loss.item()}, l1 loss: {l1_loss.item()}")
    # optimize for FACS_directions
    FACS_weights.requires_grad = False  # we will optimize this
    FACS_directions.requires_grad = True  # we will optimize this
    # RENDER THE MESH
    k_frame = 0
    V_bs, _, _ = flame_module(shape_params[k_frame:k_frame+1], FACS_based_weights[k_frame:k_frame+1, :100], pose_params=torch.concat([pose_params[k_frame:k_frame+1], FACS_based_weights[k_frame:k_frame+1, 100:103]], dim=1))
    V_gt, _, _ = flame_module(shape_params[k_frame:k_frame+1], weight[k_frame:k_frame+1, :100], pose_params=torch.concat([pose_params[k_frame:k_frame+1], weight[k_frame:k_frame+1, 100:]], dim=1))
    V_bs = V_bs.reshape((V_bs.shape[0], -1, 3))  # (Frames, Vertices, 3)
    V_gt = V_gt.reshape((V_gt.shape[0], -1, 3))  # (Frames, Vertices, 3)
    # display_a_single_mesh(V_bs[k_frame].detach().cpu().numpy(), flameBS.F, )
    display_pairs_of_meshes([V_bs[k_frame].detach().cpu().numpy()], [flameBS.F], [V_gt[k_frame].detach().cpu().numpy()], [flameBS.F], offset=0.3)
    

    for fitting_iter in range(0, EM_FACS_DIRE_ITERATIONS):
        FACS_based_weights = FACS_weights @ FACS_directions  # (Frames, 103)
        recon_loss = torch.norm(FACS_based_weights - weight, p=2, dim=-1).mean()

        exp_params = FACS_directions[:, :100]
        jaw_params = FACS_directions[:, 100:103]
        # compute frozen loss
        V_bs, _, _ = flame_module(shape_params, exp_params, pose_params=torch.concat([pose_params, jaw_params], dim=1))
        ...
        loss = recon_loss + W_REG * (lm_loss + frozen_loss * W_FROZEN)  # add the frozen loss
        loss.backward()
        direction_optimizer.step([FACS_directions], [FACS_directions.grad])
        FACS_directions.grad.zero_()  # reset grad
        if fitting_iter % 10 == 0:
            print(f"dires -> fitting_iter: {fitting_iter}, reconstruction loss: {recon_loss.item()}, frozen loss: {frozen_loss.item()}, lm loss: {lm_loss.item()}")
    losses_directions_recond.append(recon_loss.item())

    print(f"dires -> EM iter: {i}, reconstruction loss: {recon_loss.item()}, frozen loss: {frozen_loss.item()}, lm loss: {lm_loss.item()}")
    if i % 10 == 0:
    ...
    # load the 
save_root = os.path.join(surrogate_model_root_path, "EM_optimized_FACS_directions_higher_reg")
