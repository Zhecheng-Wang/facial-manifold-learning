...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        # scale_timestamps just normalizes t to be between 1 and 1000. but here we don't use in diffposetalk and face_diffuser. since we use positional encoding
        model_output = model(x, self._scale_timesteps(t), **model_kwargs) # get model output
        # # plot the jaw curves
        # jaw_curves = model_output[0, :, 50:].detach().cpu().numpy()
        ...
        # plt.savefig('temp_figures/jaw_curves_{}.png'.format(t[0]))
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            ...
            if self.model_var_type == ModelVarType.LEARNED:
                # if model predict variance
                model_log_variance = model_var_values
                model_variance = th.exp(model_log_variance)
            else:
                # if model predict range of variance
                min_log = _extract_into_tensor(
                    self.posterior_log_variance_clipped, t, x.shape # some padding for later
                ) # get the min and max log variance
                max_log = _extract_into_tensor(np.log(self.betas), t, x.shape) # some padding for later
            ...
            model_variance, model_log_variance = {
                ...
                ModelVarType.FIXED_SMALL: ( # we using this in DiffposeTalk and FaceDiffuser
                    self.posterior_variance,
            ...
            model_variance = _extract_into_tensor(model_variance, t, x.shape) # in all cases, it is dependent on the time-step
    ...
    def p_sample_with_grad(
        ...
        with th.enable_grad():
            ...
            noise = th.randn_like(x) # generate noise from a random distribution
            nonzero_mask = (
                (t != 0).float().view(-1, *([1] * (len(x.shape) - 1)))
            )  # no noise when t == 0
            if cond_fn is not None:
        ...
        sample = out["mean"] + nonzero_mask * th.exp(0.5 * out["log_variance"]) * noise # re noise the sample with the 
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            terms["output"] = [model_output, model_output]
        elif self.loss_type == LossType.DIFFPOSETALK:
            # get model output
            output_win1, output_win2, flame_output_win1, flame_output_win2 = model(x_t, self._scale_timesteps(t), **model_kwargs) 
            # =============== test =============== 
            # model.model.eval_mode()
            # output = model(x_t, self._scale_timesteps(t), **model_kwargs)
            # to_plot = output.detach().cpu().numpy()
            # noised = x_t.detach().cpu().numpy()
            # jaw = to_plot[0, :, 50:]
            # noised_jaw = noised[0, :, 50:]
            # from matplotlib import pyplot as plt
            # plt.plot(jaw)
            # plt.plot(noised_jaw)
            # # save plot
            # plt.savefig("jaw_curves_0.png")
            # A[238902390293]
            # =============== test =============== 
            
            # get target, here we make the model predice x_start
            target = {
                ModelMeanType.PREVIOUS_X: self.q_posterior_mean_variance(
                    x_start=x_start, x_t=x_t, t=t
                )[0],
            ...
            # parse the target and output for the two windows, ignoring some frames
            target_win1 = target[:, :model.model.window_size]
            target_win2 = target[:, (model.model.window_size-model.model.window_overlap_size):]


            flame_target_win1 = model.model.get_flame_vertices(model_kwargs["shape_params"], target_win1)
            flame_target_win2 = model.model.get_flame_vertices(model_kwargs["shape_params"], target_win2)



            output_win1 = output_win1[:, model.model.window_overlap_size:]
            flame_output_win1 = flame_output_win1[:, model.model.window_overlap_size:]

            # simple loss
            L_simple = self.l2_loss(target_win1, output_win1)
            L_simple += self.l2_loss(target_win2, output_win2)
            L_simple /= 2.0

            # vertex loss 
            L_vertex = self.l2_loss(flame_output_win1, flame_target_win1)
            L_vertex += self.l2_loss(flame_output_win2, flame_target_win2)
            L_vertex /= 2.0

            # vertex velocity loss
            L_velocity = self.l2_loss(flame_output_win1[:, :, 1:] - flame_output_win1[:, :, :-1], flame_target_win1[:, :, 1:] - flame_target_win1[:, :, :-1])
            L_velocity += self.l2_loss(flame_output_win2[:, :, 1:] - flame_output_win2[:, :, :-1], flame_target_win2[:, :, 1:] - flame_target_win2[:, :, :-1])
            L_velocity /= 2.0

            # vertex smoothness loss
            L_smoothness = self.l2_loss(flame_output_win1[:, :, 2:] - 2*flame_output_win1[:, :, 1:-1] + flame_output_win1[:, :, :-2], torch.zeros(flame_output_win1[:, :, 2:].shape, device=flame_output_win1.device))
            L_smoothness += self.l2_loss(flame_output_win2[:, :, 2:] - 2*flame_output_win2[:, :, 1:-1] + flame_output_win2[:, :, :-2], torch.zeros(flame_output_win2[:, :, 2:].shape, device=flame_output_win2.device))
            L_smoothness /=2.0    


            terms["simple_loss"] = L_simple
            terms["vertex_loss"] = L_vertex * 2E4
            terms["vertex_velocity_loss"] = L_velocity * 1E4
            terms["vertex_smoothness_loss"] = L_smoothness * 1E1
            terms["loss"] = terms["simple_loss"] + terms["vertex_loss"] + terms["vertex_velocity_loss"] + terms["vertex_smoothness_loss"]
            terms["output"] = [output_win1, output_win2]
...
def get_named_beta_schedule(schedule_name, num_diffusion_timesteps, scale_betas=1.):
    ...
    if schedule_name == "linear":
        ...
        # diffusion steps.
        scale = scale_betas * 1000 / num_diffusion_timesteps # if we have 1000, it will just be 1, 2 if it's 500
...
def create_gaussian_diffusion(config):
    ...
    betas = get_named_beta_schedule("cosine", steps, scale_beta) # from 0.0001 to 0.02 an array with {steps} steps
    if config["loss_type"] == "MSE" or config["model"] == "FaceDiffuser":
        loss_type = LossType.MSE
    elif config["loss_type"] == "DiffPoseTalk":
        loss_type = LossType.DIFFPOSETALK
