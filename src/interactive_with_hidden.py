...
def gui():
    ...
    for cluster_name in clusters_names:
        ...
        cluster_ids = [
            x for _, x in sorted(zip(cluster_stds, cluster_ids), reverse=True)
        ]
    ...
    for cluster_i, cluster_name in enumerate(clusters_names):
        ...
        cluster_stds = (cluster_stds - np.min(cluster_stds)) / (
            np.max(cluster_stds) - np.min(cluster_stds)
        )
        cluster_stds += 0.2
        ...
        for i, id in enumerate(cluster_ids):
            ...
            changed_bs, new_val = psim.SliderFloat(
                "##hidden" + str(id), float(weights[id]), 0.0, 1.0
            )
            psim.SameLine()
            psim.SetNextItemWidth(slider_width)
