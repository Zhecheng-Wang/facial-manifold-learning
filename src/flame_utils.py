...
def get_flame_pca_std_from_pkl(model_path):
    ...
    for key in possible_shape_keys:
        if key in model_data:
            ...
            shape_std = (
                np.sqrt(shape_values)
                if "eigenvalues" in key or "ev" in key.lower()
                else shape_values
            )
            break
...
def get_flame_blendshapes(flame_model):
    ...
    jaw_x_indices = [
        ...
        jaw_start_idx + 1,
        jaw_start_idx + 2,
    ]  # X-axis rotation influences
    jaw_y_indices = [
        jaw_start_idx + 3,
        jaw_start_idx + 4,
        jaw_start_idx + 5,
    ]  # Y-axis rotation influences
    jaw_z_indices = [
        jaw_start_idx + 6,
        jaw_start_idx + 7,
        jaw_start_idx + 8,
    ]  # Z-axis rotation influences
    ...
    jaw_pose_blendshapes = torch.stack(
        [jaw_x_blendshape, jaw_y_blendshape, jaw_z_blendshape], dim=2
    )
...
class FLAME(nn.Module):
    ...
    def __init__(self, config, device=None):
        ...
        default_eyeball_pose = torch.zeros(
            [1, 6], dtype=self.dtype, requires_grad=False
        )
        self.register_parameter(
            "eye_pose", nn.Parameter(default_eyeball_pose, requires_grad=False)
        )
        default_eyeball_pose_mat = (
            torch.eye(3, dtype=self.dtype, requires_grad=False).view(1, 9).repeat(1, 2)
        )
        self.register_parameter(
            "eye_pose_mat", nn.Parameter(default_eyeball_pose_mat, requires_grad=False)
        )
        default_neck_pose = torch.zeros([1, 3], dtype=self.dtype, requires_grad=False)
        self.register_parameter(
            "neck_pose", nn.Parameter(default_neck_pose, requires_grad=False)
        )
        default_neck_pose_mat = torch.eye(
            3, dtype=self.dtype, requires_grad=False
        ).view(1, 9)
        self.register_parameter(
            "neck_pose_mat", nn.Parameter(default_neck_pose_mat, requires_grad=False)
        )
    ...
    def forward(
        ...
        if pose2rot:
            ...
            full_pose = torch.cat(
                [
                    head_pose,
                    self.neck_pose_mat.expand(batch_size, -1),
                    pose_params[:, 9:],
                    eye_pose_params,
                ],
                dim=1,
