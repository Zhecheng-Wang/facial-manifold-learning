...
def get_face_data():
    ...
    print(
        "Base vertices:", type(base_vertices), base_vertices.dtype, base_vertices.shape
    )
    ...
    for i, name in enumerate(blendshapes.names):
        ...
        full_delta = blendshapes.blendshapes[i].astype(
            np.float32
        )  # This should be (5110, 3)
        ...
        # Calculate normal
        if np.any(non_zero_mask):
            affected_faces = np.any(
                np.isin(base_faces, np.where(non_zero_mask)[0]), axis=1
            )
