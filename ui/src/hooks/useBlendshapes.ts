...
export const useBlendshapes = () => {
  ...
  const [baseVertices, setBaseVertices] = useState<Float32Array>(new Float32Array());
  ...
  useEffect(() => {
    const loadBlendshapes = async () => {
      try {
        ...
        // Validate data
        if (!data.baseVertices || !data.baseFaces || !data.blendshapes || 
            !data.numVertices || !data.numFaces) {
        ...
        if (vertices.length !== data.numVertices * 3) {
          throw new Error(`Vertex data length mismatch: expected ${data.numVertices * 3}, got ${vertices.length}`);
        }
        if (faces.length !== data.numFaces * 3) {
          throw new Error(`Face data length mismatch: expected ${data.numFaces * 3}, got ${faces.length}`);
        }
        ...
        const convertedBlendshapes: Blendshape[] = data.blendshapes.map(bs => ({
          name: bs.name,
  ...
  return { blendshapes, weights, setWeights, loading, error, baseVertices, baseFaces };
}; 